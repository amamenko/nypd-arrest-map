  ...
  const postToMapFilterWorker = useCallback(
      (chunk, arrName, specificName, filterArr) => {
          if (mapFilterWorkerInstance) {
              postToMapFilterWorker(loadData, filteredData, filteredDataChunks, currentFilters)
      ...
      if (!mapFilterWorkerInstance) {
          ...
          const blobURL = URL.createObjectURL(
              new Blob(
                              ...
                              if (x !== filterExactName) {
                                  if (currentFilters) {
                                      if (currentFilters.length > 0) {
                                          return currentFilters.includes(x);
                                      } else {
                                          return true;
                                      }
                                  } else {
                  ...
                  const loadDataReducerFunction = (filterExactName) => {
                                      return loadData.flat().reduce((acc, curr) => {
                            ...
                            const filteredDataReducerFunction = (filterExactName) => {
                            ...
                            const boroughArrReducerFunction = (filterExactName) => {
                              return loadData.flat().reduce((acc, curr) => {
                                const currentName = (x) => {
                                    if (
                                        x[filterExactName] === "K" &&
                          ...
                          postMessage({  
                              ageGroup: loadDataReducerFunction("AGE_GROUP"),
                              raceArr: loadDataReducerFunction("PERP_RACE"),
                              boroughArr: boroughArrReducerFunction("ARREST_BORO"),
                              offenseDescriptionArr: loadDataReducerFunction("OFNS_DESC"),
                              filteredArrestCategory: filteredDataReducerFunction("LAW_CAT_CD"),
                              filteredAgeGroup,
                              filteredSexArr,
                              filteredRaceArr,
                              filteredBoroughArr,
              ...
              new Blob(
                  [
                      ...
                      (() => {
                          ...
                          let ws = new WebSocket("wss://4000-b2ce8d99-5e44-4450-8082-212ab58e5318.ws-us02.gitpod.io");
